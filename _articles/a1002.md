...
{% raw %}<div class="ques-view"><p>This time, you are supposed to find <span>$A+B$</span> where <span>$A$</span> and <span>$B$</span> are two polynomials.</p>
...
<p>For each test case you should output the sum of <span>$A$</span> and <span>$B$</span> in one line, with the same format as the input. Notice that there must be NO extra space at the end of each line. Please be accurate to 1 decimal place.</p>
